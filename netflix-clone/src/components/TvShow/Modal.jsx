...
const Modal = ({ detail, onClose, videoId,title,runtime,overview,casts,genres,movieData,Id,src,image}) => {
  ...
  console.log(movieData.number_of_seasons)
  ...
  const fetchEpisodes = async (number) => {
    const response = await fetch(`https://api.themoviedb.org/3/tv/${film.id?film.id:Id}/season/${number}?api_key=${API_KEY}&language=en-US`);
    const data = await response.json();
    setEpisode(data);
  };

  const fetchFirstEpisode = async () => {
    const response = await fetch(`https://api.themoviedb.org/3/tv/${Id}/season/1?api_key=${API_KEY}&language=en-US`);
    const data = await response.json();
    setEpisode(data)
}
          ...
          <div className="movie">
            ...
            <div className='movie-detail'>
                <div id='movie-description'>
                    <h2>{film.name?film.name:movieData.name}</h2>
                    <p><span>69% match </span>{film.number_of_seasons?film.number_of_seasons:movieData.number_of_seasons} seasons {film.number_of_episodes?film.number_of_episodes:movieData.number_of_episodes} episodes</p>
            ...
            <div className='similar-seasons'>
              <div className="episode">
               <h3>Episodes</h3> 
                {
                episode.episodes&&episode.episodes.map((item,index)=>{
                  return (<>
                    <div key={index} className='episode-detail'>
                    <img src={`https://image.tmdb.org/t/p/w500/${item.still_path}`} alt="" />
                     <div className='desc'>
           ...
           {(film.seasons && film.seasons.length > 0 ? film.seasons : movieData.seasons).map((option, index) => (
               <option value={option.season_number} key={index}>Season {index+1}</option>
                        ))}
              </select>
            </div>
            ...
            <h2>More Like This</h2>
              ...
              <div className='final'>
                ...
                <p><span>Director:</span>albert coorkie</p>
